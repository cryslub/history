...
DAT.Globe = function(container, opts) {
  ...
  function init() {
    ...
    var geometry = new THREE.SphereGeometry(radius-4*totalSize, 40, 30);
...
//        	  scene.add(mesh = wireframe(topojson.mesh(topology, topology), new THREE.LineBasicMaterial({color: 0x4682B4})));
	...
	JSZipUtils.getBinaryContent('json/wwf_terr_ecos.zip', function(err, fileData) {
	    if(err) {
...
//	  geo.colors.push(roadColors[type]);	  
  ...
  function addLines(lines) {
	  ...
	  var sphereSize = radius+0.3*totalSize;
